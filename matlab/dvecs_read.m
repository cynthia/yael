% Read a set of vectors stored in the dvec format (int + n * double)
% The function returns a set of output vector (one vector per column)
%
% Syntax: 
%   v = dvecs_read (filename)     -> read all vectors
%   v = dvecs_read (filename, n)  -> read n vectors 
%   v = dvecs_read (filename, [a b]) -> read the vectors from a to b (indices starts from 1)
function v = dvecs_read (filename, bounds)
